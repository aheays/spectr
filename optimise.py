...
class Optimiser:
    ...
    def __init__(
        ...
        self._last_construct_time = time.time() # when last constructed
        self._last_add_construct_function_time = time.time()    # when model was last modified by adding a construct function
    ...
    def add_construct_function(self,*functions):
        ...
        f()
        self._last_add_construct_function_time = time.time()
    ...
    def construct(
        ...
        for optimiser in self._get_all_suboptimisers():
            if optimiser.has_changed() or recompute_all:
                ...
                ## construct time
                optimiser.residual = []
                construct_functions = list(optimiser._construct_functions.values()) + list(optimiser._post_construct_functions.values())[::-1]
                if len(construct_functions) > 0:
                    ...
                    optimiser._last_construct_time = time.time()
    ...
    def __init__(
        ...
        if step is not None:
            ...
            if self.value != 0:
                ...
                self.step = 1e-4
        self._last_modify_value_time = time.time()
