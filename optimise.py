...
class Optimiser:
    ...
    def __init__(
            ...
            *suboptimisers,     # Optimisers
            verbose=False,
            description=None,
            **parameter_set # To add a parameter set in during initialisation
    ):
        ...
        self.format_input_functions = []        # call these functions (return strings) to build a script recreating an optimisation
        ...
        if len(parameter_set)>0:
            self.add_parameter_set(
                description=None,**parameter_set)
        # ## make an input line
        # def f():
            # retval =  f'{self.name} = Optimiser({repr(self.name)},'
            # if len(suboptimisers)>0:
                # retval += '\n    '+",".join([t.name for t in self.suboptimisers])+','
            # if len(kwargs_add_parameter_set)>0:
                # if len(kwargs_add_parameter_set)==1:
                    # retval += parameter_set.format_input()+','
                # else:
                    # retval += '\n    '+parameter_set.format_multiline()+','
    ...
    # def format_input(self, match_lines_regexp=None):
        ...
        # for optimiser in self._recurse_optimisers():
            ...
            # if len(optimiser.format_input_functions)>0:
                # lines.append('')
        # retval = '\n'.join(lines)
        # ## limit to matching lines if requested
        # if match_lines_regexp is not None:
            # retval = '\n'.join([t for t in retval.split('\n') if re.match(match_lines_regexp,t)]) # filter
        # return(retval)
    ...
    # def print_input(self,match_lines_regexp=None):
        ...
        # self.format_input_functions.append(f'{self.name}.print_input({t})')
        # retval = self.format_input(match_lines_regexp=match_lines_regexp)
        # print(retval)
