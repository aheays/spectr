...
class Optimiser:
    ...
    def add_construct_function(self,*construct_functions):
        ...
        self._construct_functions.extend(construct_functions)

    def add_monitor_function(self,*monitor_functions):
        ...
        self._monitor_functions.extend(monitor_functions)
    ...
    def save_to_directory(
        ...
        self.add_format_input_function(
            lambda directory=directory: f'{self.name}.save_to_directory({repr(directory)},trash_existing_directory={repr(trash_existing_directory)})')
        directory = tools.expand_path(directory)
        tools.mkdir(directory,trash_existing_directory=trash_existing_directory)
        ## output self and all suboptimisers into a flat subdirectory
        ## structure
        used_subdirectories = []
        for optimiser in self._get_all_suboptimisers():
            subdirectory = directory+'/'+optimiser.name+'/'
            tools.mkdir(subdirectory,trash_existing_directory=True)
    ...
    def construct(
        ...
        for optimiser in self._get_all_suboptimisers():
            if optimiser.has_changed() or recompute_all:
                ...
                optimiser.residual = []
                for f in optimiser._construct_functions:
