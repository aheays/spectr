...
class Optimiser:
    ...
    def __init__(
        ...
        ## add data/Parameters to internal store
        self._store = {}
    ...
    # @optimise_method(format_multi_line=inf)
    def add_store(self,key,val):
        """Add Parameters or other data to internal store. Only some types implemented."""
        if hasattr(val,'_this_is_a_stored_object'):
            raise Exception(f'Cannot add already stored object {repr(val)} to store.')
        ## wrap in a class that changes the repr and save in store
        class StoredObject(type(val)):
            _this_is_a_stored_object = True
            def __repr__(stored_self):
                return f'{self.name}[{repr(key)}]'
            def as_original_type(stored_self):
                if isinstance(val,(int,float,str,tuple)):
                    return val
                elif isinstance(val,(list,dict,Parameter)):
                    return type(val)(stored_self)
                else:
                    raise Exception(f"add_store for type {type(val)} not implemented")
            def __str__(stored_self):
                return str(stored_self.as_original_type())
        ...
        ## new input function
        def f():
            return f'{self.name}.add_store({repr(key)},{repr(val)})'
        self.add_format_input_function(f)

    def add_store_dict(self,keys_vals_dict=None,**keys_vals_kwargs):
        """Add Parameters or other data to internal store."""
        ## collect keys vals
        if keys_vals_dict is None:
            keys_vals_dict = {}
        keys_vals_dict |= keys_vals_kwargs
        for key,val in keys_vals_dict.items():
            self.add_store(key,val)
            self.pop_format_input_function()
        def f():
            dict_repr = tools.dict_expanded_repr(keys_vals_dict,maxdepth=1)
            retval = f'{self.name}.add_store_dict({dict_repr})'
            return retval
        self.add_format_input_function(f)    
    ...
    def export_store(self,filename):
        tools.export_dict(
            filename,
            header=r'from spectr import *',
            store={key:stored_val.as_original_type()
                   for key,stored_val in self._store.items()},)
...
class Parameter():
    ...
    def __repr__(self):
        ...
        if self.unc not in (0,nan):
            retval += ','+format(self.unc,'0.2g')
        if self.bounds[0]!=-inf  or self.bounds[1]!=inf:
