...
c       NRTOT = number of reactions from the network file + number of outgas.dat reactions
        PARAMETER (NRTOT=6615+5,NROUT=5)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        OPEN (UNIT=1,FILE='Stand-2020October_impacts',STATUS='OLD')
...
C	this subroutine reads the reactions in the database 
     ...
     &          RADNUM,JJ
        ...
        PARAMETER (NRBIS2=6615+5,NROUT=5)
...
c	to compute the rate coefficient 
     ...
     &          WLNUM,NS,NRES
        ...
        PARAMETER (NRBIS2=6615+5,WLNUM=10001,PRNUM=209+1,NPOLY=14)
...
c     Alan Heays 2020-12-07: Impact induced reaction A + B -> D + E
c     (two-body only).  The rate is assumed proportional to total
c     density. An effective reaction rate coefficient is computed
c     according to: k=A*nhttot*ZPREV/(product of reactant density),
c     where nhttot*ZPREV is particle number at this grid level. Then the
c     actual rate will be R=k*(product_reactant))=A*nhttot*ZPREV.  Then,
c     if A is defined as the induced reaction surface flux (s-1.cm-2),
c     i.e., column integrated, then R is the production rate inside this
c     vertical 1cmx1cmx(znext-zprev) grid slab.

                IF (ITYPE(J).EQ.44) THEN
c                  The following branching to account for 1/2/3-body reactions 
c                  because REACT(J,2) and REACT(J,3) are sometimes 0. Maybe
c                  could instead set REACT(J,2/3) to 1, but this might upset                   
c                  other ITYPE reactions?            
                  write (*,*) "DEBUG:"
                  write (*,*) "DEBUG:",J,A(J),B(J),NHTOT
       write (*,*) "DEBUG:",SN(REACT(J,1)),SN(REACT(J,2)),SN(REACT(J,3))


                   if (SN(REACT(J,2)).LT.1.0D-5) then
                      write(*,*) "DEBUG A"
                      K(J) = A(J)*NHTOT/B(J)
     &                        /(SN(REACT(J,1)))
                   elseif (SN(REACT(J,3)).LT.1.0D-5) then
                      write(*,*) "DEBUG B"
                      K(J) = A(J)*NHTOT/B(J)
     &                        /(SN(REACT(J,1))
     &                         *SN(REACT(J,2))) 
     ...
     &                         *SN(REACT(J,3))) 
                  ...
                  write (*,*) "DEBUG:",J,A(J),B(J),K(J),NHTOT
                ENDIF


C                IF (ITYPE(J).EQ.44) THEN
C                  K(J) = A(J)*(NHTOT*ZPREV)/B(J)
C     &                  /(SN(REACT(J,1))*SN(REACT(J,2)))
C                  write (*,*) "DEBUG:"
C       write (*,*) "DEBUG:",SN(REACT(J,1)),SN(REACT(J,2)),SN(REACT(J,3))
C                  write (*,*) "DEBUG:",J,A(J),B(J),K(J),ZPREV,NHTOT
C                ENDIF
...
C       Use a smaller lower limit for reaction type 44 (impact formation), which requires very
C       small rate coefficients sometimes.
        DO J=1,NRBIS2
           IF (ITYPE(J).EQ.44) THEN
              IF (K(J).LT.1.D-100) K(J)=0.D-50
           ELSE
...
C                if ((i.eq.6613).or.((i.eq.6614))) then
C                write(*,*) "DEBUG2:",i,Y(IR1),Y(IR2),Y(IR3)
C                endif   
...
C	for each species and put them in the increasing order 
     ...
     &          IPROD4,IY,M,I,NS2,NRBIS3,NEQ,NRES,LASTTIME,SWITCH,CONDSW
        PARAMETER (NRBIS2=6615+5,NS2=928+5+1,NRES=5)
