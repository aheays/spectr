...
tests = {
    ...
    '1Σ+-1Σ+_transition': {
         ...
         'upper':{'levels':{'B.1Σ+(v=0)':{'Tv':50000,'Bv':1.1}}}, 
         'transition_moments':{('B.1Σ+(v=0)','X.1Σ+(v=0)'):{'μv':1}}},

    '1Π-1Σ+_transition': {
         'lower':{'levels':{'X.1Σ+(v=0)':{'Tv':0,'Bv':1}}}, 
         'upper':{'levels':{'B.1Π(v=0)':{'Tv':50000,'Bv':1.1}}},
         'transition_moments':{('B.1Π(v=0)','X.1Σ+(v=0)'):{'μv':1}}},
    ...
    '2Π_2Π_transition': {
         ...
         'J_l':np.arange(0.5,31),
         'lower':{'levels':{'X.2Π(v=0)':{'Tv':100,'Bv':+1.6,'Av':+120,'γv':-0.01},},}, 
         'upper':{'levels':{'B.2Π(v=0)':{'Tv':50000,'Av':100,'Bv':1.1,'Dv':6e-6,'γv':10,'γDv':-1e-4}}}, 
         'transition_moments':{('B.2Π(v=0)','X.2Π(v=0)'):{'μv':1}}},

    '3Σ+-3Σ+_transition': {
         'lower':{'levels':{'X.3Σ+(v=0)':{'Tv':0,'Bv':1,'λv':1,'γv':0.01},},}, 
         'upper':{'levels':{'C.3Σ+(v=0)':{'Tv':49950,'Bv':1.4,'Dv':1e-6,'λv':1,'λDv':1e-4,'γv':0.01,'γDv':1e-4},}},
         'transition_moments':{('C.3Σ+(v=0)','X.3Σ+(v=0)'):{'μv':1}}},

    '3Π-3Σ+_transition': {
         'lower':{'levels':{'X.3Σ+(v=0)':{'Tv':0,'Bv':1,'λv':1,'γv':0.01}}}, 
         'upper':{'levels':{'B.3Π(v=0)':{'Tv':50000,'Bv':1.1,'Av':10}}}, 
         'transition_moments':{('B.3Π(v=0)','X.3Σ+(v=0)'):{'μv':1}}},

    '3Π-3Π_transition': {
         'lower':{'levels':{'X.3Π(v=0)':{'Tv':0,'Bv':1,'Av':5,'λv':1,'γv':0.01},},}, 
         'upper':{'levels':{'B.3Π(v=0)':{'Tv':50000,'Bv':1.1,'Av':10}}},
         'transition_moments':{('B.3Π(v=0)','X.3Π(v=0)'):{'μv':1}}},

     '3Δu_3Πg_transition': { # 14N2 W-B(3-1) from western201 -- not working  
          'species':'[14N]2',
          'lower':{'levels':{'B.3Πg(v=1)':{'Tv':11255.202248,'Bv':1.61055197,'Dv':5.89799e-06,'Hv':-3.94391e-13,'Av':42.1932,'ADv':-0.000421635,'λv':-0.203999,'λDv':-6.34857e-07,'γv':-0.00367735,'ov':1.15017,'pv':0.00431673,'qv':8.34856e-05,},},},
          'upper':{'levels':{'W.3Δu(v=3)':{'Tv':13989.734287,'Bv':1.41063813,'Dv':5.65923e-06,'Hv':-6.54037e-13,'Av':5.6636,'ADv':3.92926e-06,'λv':0.67833,'λDv':1.51497e-06,'γv':-0.00288008,},}},
          'transition_moments':{('W.3Δu(v=3)','B.3Πg(v=1)'):{'μv':1}}},

    '3Π_3Σ+_intensity_interference': {
        'species':'[32S][16O]',
        'lower': {'levels':{'X.3Σ+(v=0)':{'Tv':0, 'Bv':1, 'λv':1, 'γv':0.01},},},
        'upper': {'levels':{'B.3Π(v=0)': {'Tv':50000, 'Bv':1.1, 'Av':10,}, 'C.3Σ+(v=0)':{'Tv':49950, 'Bv':1.4, 'λv':1, 'γv':0.01,},}, 'couplings':{('B.3Π(v=0)','C.3Σ+(v=0)'): {'ξv':0.1, 'ηv':10},}},
        'transition_moments':{('B.3Π(v=0)','X.3Σ+(v=0)'):{'μv':1}, ('C.3Σ+(v=0)','X.3Σ+(v=0)'):{'μv':1}}},

    '3Π-3Σ-_transition': {
         'species':'[32S][16O]',
         'lower':{'levels':{'X.3Σ-(v=0)':{'Tv':0, 'Bv':convert.units(21523.555878,'MHz','cm-1'), 'Dv':convert.units(33.915261e-3,'MHz','cm-1'), 'Hv':convert.units(-6.974e-9,'MHz','cm-1'), 'λv':convert.units(158254.392,'MHz','cm-1'), 'λDv':convert.units(0.306259,'MHz','cm-1'), 'λHv':convert.units(0.478e-6,'MHz','cm-1'), 'γv':convert.units(-168.3043,'MHz','cm-1'), 'γDv':convert.units(-0.52545e-3,'MHz','cm-1'),}}},
         'upper':{'levels':{'C.3Π(v=0)':{'Tv':44151.8,'Bv':0.567,'Dv':1.2e-5,'Av':-181.4,'λv':1.0,'ov':0.98,'γv':-0.2,}}},
         'transition_moments':{('C.3Π(v=0)','X.3Σ-(v=0)'):{'μv':0.5}}},
    ...
    '32S16O_B05_C00_d01': {
        ...
        'upper':{
         ...
         'transition_moments':{('B.3Σ-(v=5)','X.3Σ-(v=0)'):{'μv':1},
    ...
    '32S16O_B01_A10_App14_simplified': { # from liu_ching-ping2006, leave out <B|LS|App>_because of phase proble: 
         'species':'[32S][16O]',
         'lower':{'levels':{'X.3Σ-(v=0)':{ 'Tv':573.79105, 'Bv':0.71794874, 'Dv':1.1313242e-06, 'Hv':-2.1611497e-13, 'λv':5.2784506, 'λDv':1.0213915e-05, 'λHv':1.6510442e-11, 'γv':-0.0056139085, 'γDv':-1.7618215e-08,}}},
         'upper':{
             'levels':{
        ...
        'transition_moments':{('B.3Σ-(v=1)','X.3Σ-(v=0)'):{'μv':1}}},
