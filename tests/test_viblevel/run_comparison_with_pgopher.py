...
tests = [
    ...
    # dict(name='1Π_1Σ+_intensity_interference',
         ...
         # upper_coupling=({'name1':'B.1Π(v=0)','name2':'C.1Σ+(v=0)','ξv':1},),
         # transition_moments=({'name_u':'B.1Π(v=0)','name_l':'X.1Σ+(v=0)','μv':1},  
    ...
    dict(name='3Π_3Σ+_intensity_interference',
         ...
         upper_coupling=({'name1':'B.3Π(v=0)','name2':'C.3Σ+(v=0)','ξv':0.1},
                         {'name1':'B.3Π(v=0)','name2':'C.3Σ+(v=0)','ηv':10},),
         transition_moments=({'name_u':'B.3Π(v=0)','name_l':'X.3Σ+(v=0)','μv':1},
    ...
    # dict(name='32S16O_B05_C00_d01',
         ...
         # upper_coupling=(
             # {'name1':'C.3Π(v=0)','name2':'B.3Σ-(v=5)','ξv':0.32,'ξDv':-4.1e-4},
             # {'name1':'C.3Π(v=0)','name2':'B.3Σ-(v=5)','ηv':2.4,'ηDv':0.015},
             # {'name1':'C.3Π(v=0)','name2':'d.1Π(v=1)','ηv':8.7},
         # ),
    ...
    # dict(name='32S16O_B01_A10_App14_simplified', # from liu_ching-ping2006, leave out <B|LS|App>_because of phase problem
         ...
         # ),
         # upper_coupling=(
             # {'name1':'B.3Σ-(v=1)','name2':'App.3Σ+(v=14)','pv':1.87},
             # {'name1':'B.3Σ-(v=1)','name2':'A.3Π(v=10)','ξv':-0.020},
         # ),
     ...
     # dict(name='32S16O_B01_A10_App14', # from liu_ching-ping2006 -- currently bad phases!!!
         ...
         # upper_levels=({'name':'B.3Σ-(v=1)', 'Tv':41991.97, 'Bv':0.49426 , 'Dv':1.61e-6 , 'λv':3.20    , 'γv':-1.37e-2,},
                       ...
                       # {'name':'App.3Σ+(v=14)', 'Tv':42007.35  , 'Bv':0.256     , 'λv':-2.66     , 'γv':0.81      ,},),
         # upper_coupling=(
             # {'name1':'B.3Σ-(v=1)','name2':'App.3Σ+(v=14)','pv':-0.161},
             # {'name1':'B.3Σ-(v=1)','name2':'A.3Π(v=10)','ξv':-0.020},
             # {'name1':'B.3Σ-(v=1)','name2':'App.3Σ+(v=14)','ηv':1.87},
         # ),
         # transition_moments=({'name_u':'B.3Σ-(v=1)','name_l':'X.3Σ-(v=0)','μv':1},),),
    ...
    # dict(name='32S16O_B01_App14', # test phases of <B|Sun|App> and <B|LS|App> -- they do not agree. Change the sign fo one to correctly agree with the other half of the lines
         ...
         # upper_coupling=(
             # {'name1':'B.3Σ-(v=1)','name2':'App.3Σ+(v=14)','pv':1.87},
             # {'name1':'B.3Σ-(v=1)','name2':'App.3Σ+(v=14)','ηv':1},
         # ),
         # transition_moments=({'name_u':'B.3Σ-(v=1)','name_l':'X.3Σ-(v=0)','μv':1},),),
...
for itest,test in enumerate(tests):
    ...
    line_mod = transition.line
    line_mod.remove_match(μ=0)
    

    ## pgopher
    line_pgo = lines.LinearDiatomic(species=test['species'])
    ...
    # dict(name='3Π_3Σ+_intensity_interference',
    # line_pgo['Σ_u'] = line_pgo['Fi_u']-2
    # line_pgo['Σ_l'] = 1
    # line_pgo['Σ_l'][line_pgo['Fi_l']==1] = 0
    # a,b = dataset.get_common(line_pgo.get_upper_level(reduce_to='first'),line_mod.get_upper_level(reduce_to='first'))
    qfig(99)
    # line_pgo.plot('J_l','E_l',zkeys=[],ax=gca(),color='red',ls='',marker='o')
    # line_mod.plot('J_l','E_l',zkeys=[],ax=gca(),color='blue',ls='',marker='x')
    line_pgo.plot('J_u','E_u',zkeys=[],ax=gca(),color='red',ls='',marker='o')
    line_mod.plot('J_u','E_u',zkeys=[],ax=gca(),color='blue',ls='',marker='x')
    # a['E'] -= b['E']
    # a.plot('J','E')
