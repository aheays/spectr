...
## harmonic
ω = 1e14
VSI = 1/2*convert.units(μ,'amu','kg')*ω**2*convert.units(R-2,'Å','m')**2
Vharmonic = convert.units(VSI,'J','cm-1')
ΔE = constants.hbar*ω
ωe_harmonic = convert.units(ΔE,'J','cm-1')
## morse
De = 50000                      # cm-1
Re = 2                          # Å
a = 2                           # Å-1
Vmorse = De*(np.exp(-2*a*(R-Re))-2*np.exp(-a*(R-Re)))+De # Morse potential (cm-1)
ν0 = convert.units(a,'m','Å')/(2*constants.pi)*np.sqrt(2*convert.units(De,'cm-1','J')/convert.units(μ,'amu','kg'))
ωe_morse =  convert.units(constants.h*ν0,'J','cm-1')
